<template>
	<admin-layout>
		...
		>
			<a-tab-pane :key="page.fullPath" v-for="page in pageList" class="tab-page">
				<span slot="tab" :pagekey="page.fullPath">{{ pageName(page) }}</span>
			</a-tab-pane>
		</a-tabs>
		<div :class="['tabs-view-content', layout, pageWidth]" :style="`margin-top: ${multiPage ? -24 : 0}px`">
			<page-toggle-transition :disabled="animate.disabled" :animate="animate.name" :direction="animate.direction">
				<a-keep-alive v-if="multiPage" v-model="clearCaches"><router-view ref="tabContent" :key="$route.fullPath" /></a-keep-alive>
				<router-view v-else />
